...
int main(){
	...
	for(int i = 0 ; i < n; i++){
		...
		for(int k = 0 ; k < n;k ++){
			if(i == k)
				continue;
			adj_list[i].push_back({trails[i][k].length, k});
		}
...
int next_vertex(ant c_ant){
	...
	int added = 0;
	int num = 0;
	for(auto pp : adj_list[c_ant.current_vert]){
		if(num == ADJ_MAX)
			break;
		num++;
		int i = pp.second;
	...
	if(added > 0){
		...
		for(int i = 0; i < n; i++){
			...
			if(trails[c_ant.current_vert][i].feromones > best){
				best = trails[c_ant.current_vert][i].feromones;
				node = i;
			}
		}
		return node;
